...
class SMRealTimeAdapter(RealTimeGameAdapter):
    """Sol/Luna alpha.42: party/PC/HP live + progreso de Kahunas validado."""

    key = "sm-azahar-rpc-roles"
    game_key = "sm"
    ...
    def _save_party_transition_diagnostic(
        ...
        expected_size = 6 * SM_PARTY_STRIDE
        ...
        zones = (
            ...
            ("runtime_gap", PK7_STORED_SIZE, SM_PARTY_STATS_OFFSET),
            ("stats", SM_PARTY_STATS_OFFSET, SM_PARTY_STATS_OFFSET + SM_PARTY_STATS_SIZE),
            ("runtime_tail", SM_PARTY_STATS_OFFSET + SM_PARTY_STATS_SIZE, SM_PARTY_STRIDE),
        )
        slot_records: list[dict[str, object]] = []
        for slot in changed_slots:
            start = (slot - 1) * SM_PARTY_STRIDE
            before = bytes(before_region[start:start + SM_PARTY_STRIDE])
            after = bytes(after_region[start:start + SM_PARTY_STRIDE])
            ...
            unknown_offsets = [
                ...
                if PK7_STORED_SIZE <= offset < SM_PARTY_STATS_OFFSET
                or SM_PARTY_STATS_OFFSET + SM_PARTY_STATS_SIZE <= offset < SM_PARTY_STRIDE
            ]
            slot_records.append({
                "slot": int(slot),
                "address": f"0x{int(party_base) + (slot - 1) * SM_PARTY_STRIDE:08X}",
        ...
        payload = {
            "format": "rolerun-sm-party-runtime-transition-v1",
            "version": APP_VERSION,
            "created_at": datetime.now().isoformat(timespec="milliseconds"),
            "purpose": (
                "Comparar un swap REAL hecho dentro de Pokémon Sol/Luna contra el layout sparse de lectura. "
            ...
            "stats_size": int(SM_PARTY_STATS_SIZE),
            "changed_slots": slot_records,
        }
        try:
            LOG_DIR.mkdir(parents=True, exist_ok=True)
            text = json.dumps(payload, ensure_ascii=False, indent=2)
            (LOG_DIR / "sm_party_runtime_transition_latest.json").write_text(text, encoding="utf-8")
            stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
            (LOG_DIR / f"sm-party-runtime-{stamp}.json").write_text(text, encoding="utf-8")
        except OSError:
            pass
    ...
    def _convert(
        ...
        return RealTimeSnapshot(
            ...
            profile="SM-alpha.42-kahuna-progress",
            ...
            diagnostics=(
                ...
                LiveDiagnostic(
                    ...
                    "SM party 0x104 + mirror + BoxPokemon 0xE8 · rollback",
                ),
            )
    ...
    def capture_monitor(
        ...
        *,
        ...
        try:
            ...
            probe = None
            probe_error = str(exc) or "Fallo al leer PS de batalla SM."
        else:
            probe_error = ""
        ...
        if probe is None:
            ...
            diagnostic = LiveDiagnostic(
                ...
                probe_error or "La sonda de batalla SM no respondió; sigue activo el fallback post-combate.",
            ...
            # `SMLiveReader.read_battle_opponent_team_size`-, nunca puede
            ...
            if state == "battle" and not validated:
                ...
                message = (
                    ...
                    "Fuera de combate · flag SM validado."
                )
            diagnostic = LiveDiagnostic(
                "battle", level, message,
                "SM battle flag + displayed/max/actual HP", elapsed,
            )
        ...
        badge_diagnostic = LiveDiagnostic(
            ...
            str(badge_source or "SM · Z-Crystals"),
            badge_elapsed,
        )
    ...
    def capture_full(
        ...
        *,
        ...
        if probe is None:
            ...
            diagnostic = LiveDiagnostic(
                "battle", DiagnosticLevel.WARNING,
                probe_error or "La sonda de batalla SM no respondió durante la sincronización inicial.",
            ...
            diagnostic = LiveDiagnostic(
                "battle", level, message,
                "SM battle flag · baseline inicial", elapsed,
            )
    ...
    def read_pc(self, anchors, *, box_count: int | None = None, box_slot_count: int | None = None):
        if self._last_game is None:
            raise SMLiveError("Sol/Luna todavía no tiene una captura viva validada. Entra al overworld y pulsa F5.")
        if self._last_save_path is None:
            raise SMLiveError("Falta el main testigo para demostrar el PC vivo de Sol/Luna.")
        if box_count is None or box_slot_count is None:
            raise SMLiveError("Faltan las dimensiones de cajas que PKHeX ha leído del guardado real.")
    ...
    def read_tm_inventory(self, saved_items=None, *, save_path: Path | str | None = None):
        ...
        if save_path is None:
            raise SMLiveError("Falta el main testigo para demostrar la mochila viva de Sol/Luna.")
    ...
    def _apply_serialized_role_changes(
        ...
        try:
            ...
            if compensation_errors:
                raise SMLiveError(
                    "El intercambio de roles de Sol/Luna falló y RoleRun no pudo "
                    "confirmar toda la compensación: " + "; ".join(compensation_errors)
                    + f". Error original: {exc}"
                ) from exc
            if applied:
                raise SMLiveError(
                    f"El intercambio de roles de Sol/Luna falló en el paso {len(applied) + 1}; "
        ...
        if process is None:
            raise SMLiveError("No se llegó a aplicar ningún cambio de rol en Sol/Luna.")
    ...
    def runtime_state(self) -> dict[str, object]:
        return {
            ...
            "capabilities": {
                "party": "read-live-validated",
                "roles": "read-write-alpha.15-content-validated-host-fcram",
                "moves_write": "read-write-alpha.15-content-validated-host-fcram",
                "battle": "read-live-alpha.41-flag-displayed-hp-maxhp-validated",
                "progress": "read-live-alpha.42-kahuna-zcrystals-validated",
                "pc": "read-write-alpha.33-swap-only-full-encrypted-party-host-guest-verified",
                "tm_inventory": "read-live-alpha.19-rom-effective-plus-structural-bag",
                "tm_teach": "write-live-alpha.19-pk7-reusable",
                "inventory_utilities": "read-write-alpha.15-pkhex-structural-witness-host-fcram",
            },
        }
