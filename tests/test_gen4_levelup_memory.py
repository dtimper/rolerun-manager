"""Parcheo de la tabla de aprendizajes de cuarta dentro de la RAM de melonDS.

A diferencia de quinta, aquí movimiento y nivel comparten los mismos dos
bytes (ver ``gen4_levelup_moves.py``), así que escribir exige RE-EMPAQUETAR
con el nivel esperado, no solo sustituir un campo separado. El escritor solo
escribe si el nivel que decodifica la entrada actual sigue siendo el
esperado: un nivel distinto significa que ahí ya no está la tabla que
creíamos, y se salta esa entrada en vez de escribir a ciegas.
...
from app import gen4_levelup_memory as memoria
from app.gen4_levelup_memory import Gen4RomImage
from app.gen4_levelup_moves import pack_entry
...
class ApplyTests(unittest.TestCase):
    ...
    def _imagen(self, entradas: list[tuple[int, int]]):
        ...
        offsets = [64 + indice * 2 for indice in range(len(entradas))]
        return Gen4RomImage(1, self.BASE, __import__("pathlib").Path("falsa.nds")), ram, offsets

    def test_escribe_reempaquetando_con_el_nivel_esperado(self) -> None:
        ...
        self.assertEqual(
            ram.leer(self.BASE + offsets[0], 2), struct.pack("<H", pack_entry(304, 5)),
        )
        # La otra entrada no se toca.
        self.assertEqual(
            ram.leer(self.BASE + offsets[1], 2), struct.pack("<H", pack_entry(45, 9)),
        )
    ...
    def test_un_nivel_distinto_del_esperado_no_se_escribe(self) -> None:
        ...
        self.assertEqual(
            ram.leer(self.BASE + offsets[0], 2), struct.pack("<H", pack_entry(33, 5)),
        )
    ...
    def test_read_entry_devuelve_movimiento_y_nivel_desempaquetados(self) -> None:
        imagen, ram, offsets = self._imagen([(33, 5)])
        with _parchea_kernel(ram):
            self.assertEqual(imagen.read_entry(offsets[0]), (33, 5))

    def test_el_nivel_se_conserva_aunque_cambie_el_movimiento(self) -> None:
        """La prueba central del formato empaquetado: el nivel NO es un campo

        aparte -si el escritor lo olvidara, esta prueba lo pillaría-.
        """
        imagen, ram, offsets = self._imagen([(33, 47)])
        with _parchea_kernel(ram):
            imagen.apply({offsets[0]: 304}, expected_levels={offsets[0]: 47})
            mov, nivel = imagen.read_entry(offsets[0])
        self.assertEqual((mov, nivel), (304, 47))
...
class AnchorTests(unittest.TestCase):
    def test_el_ancla_es_la_especie_con_mas_entradas(self) -> None:
        entradas = {
            1: ((33, 1, 100), (45, 3, 102)),
            2: ((33, 1, 200), (45, 3, 202), (22, 9, 204), (73, 12, 206)),
            3: (),
        }
        especie, inicio, longitud = memoria._anchor_species(entradas)
        # 4 entradas * 2 bytes (ENTRY_SIZE de cuarta) = 8, no 16 como en quinta.
        self.assertEqual((especie, inicio, longitud), (2, 200, 8))

    def test_una_tabla_vacia_se_rechaza_en_vez_de_buscar_a_ciegas(self) -> None:
        with self.assertRaises(memoria.Gen4RomImageError):
